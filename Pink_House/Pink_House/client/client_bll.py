...
class Client(object):
    ...
    def __connect(self):
        try:
            ...
            self.sockfd.connect(("127.0.0.1", 8888))
...
class ClientSend:
    ...
    def register(self, uid, upwd, upwd1, uname, uphone):
        """
        ...
        data = {"style": "R", "uid": uid, "uname": uname, "upwd": upwd, "uphone": uphone}
    ...
    def get_rooms(self, uid):  #获取群聊列表
        data = {"style": "S1", "uid": uid}
        request = json.dumps(data).encode()
        self.clientor.send(request)
        msg = json.loads(self.clientor.recv(2048).decode())
        if not msg:
    ...
    def get_off_msg(self, uid):  # 获取离线消息
    ...
    def del_friend(self, uid, fuid):
        ...
        for i in fuid:
            if i not in "0123456789":
                return "请输入好友账号"
        data = {"style": "De", "uid": uid, "fuid": fuid}
        request = json.dumps(data).encode()
        print('发送请求',request)
        self.clientor.send(request)
    ...
    def get_history_msg(self, uid, fuid):  #聊天记录
        data = {"style": "H", "uid": uid, "fuid": fuid}
        request = json.dumps(data).encode()
        self.clientor.send(request)
    ...
    def create_room(self, uid, rid, rname):  # 创建群聊
        if len(rid) > 5 or rid == '':
            return "请输入1-5位数字群号"
        elif len(rname) < 1 or len(rname) >20:
            return "请输入1-20位群名称"
        for i in rid:
            if i not in "0123456789":
                return "请输入数字群号"
...
class ClientRecv:
    ...
    def start(self, msg):
        ...
        elif msg["style"] == "De": #删除好友结果
            self.friend_delete_result()
        elif msg["style"] == "N":  # 好友私聊信息
            self.recv_chat_msg()
        elif msg["style"] == "H":  #  查询聊天记录
            self.history_msg()
        elif msg["style"] == "B":  # 是否存在该好友
            self.isfriend()
        elif msg["style"] == "C":  # 创建群聊结果
            self.create_room_result()
        elif msg["style"] == "J":  # 加入群聊结果
            self.add_room_result()
    ...
    def friend_delete_result(self): # 好友删除结果
        del_re.append(self.msg)
    ...
    def history_msg(self,):
        his_msg.append(self.msg)
    def isfriend(self):
        is_friend.append(self.msg["re"])

    def create_room_result(self):  # 创建群聊结果
        create_room.append(self.msg)

    def add_room_result(self):  #加入群聊结果
        add_room_re.append(self.msg)
        print(add_room_re)
# if __name__ == '__main__':
#     c = ClientSend()
#     c.del_friend("123456", "222222")
#     c.recvmsg()