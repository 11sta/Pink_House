...
# 服务器地址
ADDR = (config.SERVER_IP, config.PORT)
...
def do_request(c, addr):
    ...
    while True:
        try:
            data = c.recv(4096).decode()  #读取客服端发来的消息
        except Exception as e:  #出现异常退出
            re.do_user_exit(c)
            return
        if not data:
            re.do_user_exit(c)
        ...
        if request['style'] == 'Q':
            ...
            # 登录后给客户端的初始化 ，加载好友列表、群聊列表
            re.do_load_friend_list(c, request)
        elif request['style'] == 'S1':
            re.do_load_rooms_list(c, request)
        ...
        elif request['style'] == 'De':
            # 删除好友
            re.do_deletefriend(c, request)
        elif request['style'] == 'C':
            # 创建群聊房间
            re.do_create_room(c, request)

        elif request['style'] == 'H':
            #查询聊天记录
            re.do_history_msg(c, request)
